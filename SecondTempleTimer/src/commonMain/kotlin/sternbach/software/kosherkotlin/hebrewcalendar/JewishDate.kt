...
open class JewishDate : Comparable<JewishDate> {
    ...
    constructor(
        hebrewYear: Int,
        hebrewMonth: Int,
        hebrewDayOfMonth: Int,
    ) : this(hebrewYear, HebrewMonth.getMonthForValue(hebrewMonth), hebrewDayOfMonth)
...
//        require(LocalDate.get(LocalDate.ERA) != GregorianLocalDate.BC) { ("LocalDates with a BC era are not supported. The year ${LocalDate.get(LocalDate.YEAR)} BC is invalid.") } //TODO how should this be dealt with?
     ...
     */
    fun setJewishDate(hebrewLocalDate: HebrewLocalDate): JewishDate =
        setJewishDate(
    ...
    fun forward(
        ...
        require(field == DateTimeUnit.DAY || field == DateTimeUnit.MONTH || field == DateTimeUnit.YEAR) {
            "Unsupported field was passed to Forward. Only DateTimeUnit.DATE, DateTimeUnit.MONTH or DateTimeUnit.YEAR are supported."
        }
        ...
        when (field) {
            DateTimeUnit.DAY -> {
                ...
                for (i in 0 until amount) {
                    ...
                    hebrewLocalDate =
                        if (hebrewLocalDate.dayOfMonth == daysInJewishMonth) {
                            ...
                            when (hebrewLocalDate.month) {
                                ...
                                -> HebrewLocalDate(hebrewLocalDate.year, HebrewMonth.NISSAN, 1)
    ...
    private fun forwardJewishMonth(amount: Int) {
        ...
        for (i in 0 until amount) {
            if (hebrewLocalDate.month == HebrewMonth.ELUL) {
                ...
                setJewishMonth(hebrewLocalDate.month.nextMonth)
            }
        }
    }
    ...
    fun back(): JewishDate {
        ...
        // change Jewish date
        hebrewLocalDate =
            if (hebrewLocalDate.dayOfMonth == 1) { // if first day of the Jewish month
                when (hebrewLocalDate.month) {
                    HebrewMonth.NISSAN ->
                        HebrewLocalDate(
                            hebrewLocalDate.year,
    ...
    fun getGregorianDate(): LocalDate {
        val year = gregorianYear
        val month = gregorianMonth
        val dayOfMonth = gregorianDayOfMonth
    ...
    companion object {
         ...
         */
        internal fun getLastDayOfGregorianMonth(
            month: Int,
            year: Int,
        ): Int =
            when (month) {
         ...
         */
        private fun addDechiyos(
            year: Int,
            moladDay: Int,
            moladParts: Int,
        ): Int = addDechiyos(year.toLong(), moladDay.toLong(), moladParts).toInt()
        ...
        private fun addDechiyos(
            ...
            if ((
                    (moladParts >= 19440) ||
                        // Dechiya of Molad Zaken - molad is >= midday (18 hours * 1080 chalakim)
                        (
                            (
                                ((moladDay % 7L) == 2L) &&
                                    // start Dechiya of GaTRaD - Ga = is a Tuesday
                                    (moladParts >= 9924) &&
                                    // TRaD = 9 hours, 204 parts or later (9 * 1080 + 204)
                                    !year.isJewishLeapYear
                            )
                        ) ||
                        // of a non-leap year - end Dechiya of GaTRaD
                        (
                            (
                                ((moladDay % 7L) == 1L) &&
                                    // start Dechiya of BeTuTaKFoT - Be = is on a Monday
                                    (moladParts >= 16789) &&
                                    // TRaD = 15 hours, 589 parts or later (15 * 1080 + 589)
            ...
            if ((
                    ((roshHashanaDay % 7L) == 0L) ||
                        // If Rosh HaShanah would occur on Sunday,
                        ((roshHashanaDay % 7L) == 3L) ||
                        // or Wednesday,
         ...
         * @return the number of chalakim (parts - 1080 to the hour) from the original hypothetical Molad Tohu
         */
        private fun getChalakimSinceMoladTohu(
            year: Long,
            month: HebrewMonth,
        ): Long {
            // Jewish lunar month = 29 days, 12 hours and 793 chalakim
        ...
        private fun validateJewishDate(
            ...
            require(!(chalakim < 0 || chalakim > 17)) {
                "Chalakim/parts < 0 or > 17 can't be set. $chalakim is invalid. For larger numbers such as 793 (TaShTzaG) break the chalakim into minutes (18 chalakim per minutes, so it would be 44 minutes and 1 chelek in the case of 793 (TaShTzaG)"
            }
        }
         ...
         */
        internal fun getDaysInJewishMonth(
            month: HebrewMonth,
            year: Int,
        ): Int = getDaysInJewishMonth(month, year.toLong())
         ...
         * @return the number of days
         */
        fun getDaysSinceStartOfJewishYear(
            year: Long,
            month: HebrewMonth,
            dayOfMonth: Int,
        ): Int {
