...
internal class JewishCalendar : JewishDate {
    ...
    val specialShabbos: Parsha
        get() {
            if (gregorianLocalDate.dayOfWeek == DayOfWeek.SATURDAY) {
                if ((hebrewLocalDate.month == HebrewMonth.SHEVAT && !isJewishLeapYear) ||
                    (hebrewLocalDate.month == HebrewMonth.ADAR && isJewishLeapYear)
                ) {
...
// push off Tzom Gedalia if it falls on Shabbos// if Tisha B'av falls on Shabbos, push off until Sunday
    ...
    val isErevYomTovSheni: Boolean
        get() =
            (hebrewLocalDate.month == HebrewMonth.TISHREI && jewishDayOfMonth == 1) ||
                (
                    !inIsrael &&
                        (
                            (
                                (hebrewLocalDate.month == HebrewMonth.NISSAN && (jewishDayOfMonth == 15 || jewishDayOfMonth == 21)) ||
                                    (
                                        hebrewLocalDate.month == HebrewMonth.TISHREI &&
                                            (jewishDayOfMonth == 15 || jewishDayOfMonth == 22)
                                    ) ||
            ...
            return (month == HebrewMonth.CHESHVAN || month == HebrewMonth.IYAR) &&
                (
                    dayOfWeek == DayOfWeek.MONDAY &&
                        day in 5..17/*4 < day < 18*/ ||
                        dayOfWeek == DayOfWeek.THURSDAY &&
                        day in 8..13 // 7 < day < 14
                )
        }
    ...
    val dayOfChanukah: Int
        get() =
            if (isChanukah) {
                if (hebrewLocalDate.month == HebrewMonth.KISLEV) {
                    jewishDayOfMonth - 24
                } else /*teves*/ if (isKislevShort) {
                    jewishDayOfMonth + 5
                } else {
                    jewishDayOfMonth + 6
                }
    ...
    val isShabbosMevorchim: Boolean
        get() =
            gregorianLocalDate.dayOfWeek == DayOfWeek.SATURDAY &&
                jewishDayOfMonth >= 23 &&
                jewishDayOfMonth <= 29 &&
    ...
    val dayOfOmer: Int
        get() {
            ...
            // if Nissan and second day of Pesach and on
            return if (month == HebrewMonth.NISSAN && day >= 16) {
                day - 15
            } else if (month == HebrewMonth.IYAR) {
                day + 15 // if Iyar
            } else if (month == HebrewMonth.SIVAN && day < 6) {
                day + 44 // if Sivan and before Shavuos
            } else {
                -1 // not a day of the Omer
            }
        }
...
// Har Habayis latitude
        ...
        get() {
            ...
            return LocalDateTime(
                ...
                LocalTime(
                    ...
                    molad.moladMinutes,
                    moladSeconds.toInt(),
                    (1000 * (moladSeconds - moladSeconds.toInt()))
                        .toInt()
                        .milliseconds.inWholeNanoseconds
                        .toInt(),
                ),
    ...
    companion object {
         ...
         */
        val parshalist: Array<Array<Parsha>> =
            arrayOf(
                arrayOf(
                    NONE,
                    Parsha.VAYEILECH,
